    ...
    if (rayCollision && hitCollision)
    {
      ...
      if (n > 0)
      {
        ...
        if (contact.depth < shape->GetLength())
        {
          ...
          // Compute Contact's Magnitude
          dVector3 contact_normal = {contact.normal[0], contact.normal[1],
                                     contact.normal[2], contact.normal[3]};
          
          dReal contact_mag = sqrt(pow(contact_normal[0],2) + 
                                   pow(contact_normal[1],2) + 
                                   pow(contact_normal[2],2)   );
          ...
          dVector3 ray = {ray_vec.x, ray_vec.y, ray_vec.z, 0};
          
          // Compute angle between two vectors
          dReal theta = acos(dDOT(contact_normal, ray) / 
                             (contact_mag * ray_mag));
